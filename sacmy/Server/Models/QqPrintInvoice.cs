using System;
using System.Collections.Generic;

namespace sacmy.Server.Models;

public partial class QqPrintInvoice
{
    public int Id { get; set; }

    public int? Idd { get; set; }

    public DateTime? Datee { get; set; }

    public string? Customer { get; set; }

    public string? Address { get; set; }

    public string? Mob { get; set; }

    public string? Notes { get; set; }

    public DateTime? Now { get; set; }

    public string? Treasurer { get; set; }

    public string? Uuser { get; set; }

    public double? Discount { get; set; }

    public decimal? Payed { get; set; }

    public decimal? Remaing { get; set; }

    public decimal? Tootal { get; set; }

    public string? Mandob { get; set; }

    public double? ManCcount { get; set; }

    public decimal? Ijraaa { get; set; }

    public decimal? Ijraaa2 { get; set; }

    public string? Driver { get; set; }

    public string? CarNo { get; set; }

    public string? DriverMob { get; set; }

    public string? Subb { get; set; }

    public decimal? Hamalya { get; set; }

    public int? Secode { get; set; }

    public string? Codd { get; set; }

    public string? Itemm { get; set; }

    public double? Smqtty { get; set; }

    public double? Smweznn { get; set; }

    public string? QiyasUnit { get; set; }

    public double? Countt { get; set; }

    public decimal? Prise { get; set; }

    public decimal? Smtotal { get; set; }

    public string? SubbB { get; set; }

    public string? CodIqd { get; set; }

    public string? Name { get; set; }

    public string? NameEng { get; set; }

    public string? Description { get; set; }

    public string? Descriptioneng { get; set; }

    public string? Expr1 { get; set; }

    public string? Mob1 { get; set; }

    public string? Mob2 { get; set; }

    public string? Mob3 { get; set; }

    public string? Mob4 { get; set; }

    public string? Email { get; set; }

    public string? Web { get; set; }

    public string? Barcod { get; set; }

    public string? BoxContain { get; set; }

    public DateTime? Date { get; set; }

    public double? Mkaab { get; set; }

    public double? Smmkaab { get; set; }

    public double? Sumqtty { get; set; }

    public double? Summwezn { get; set; }

    public double? Smmkaabb { get; set; }

    public double? SummweznTotal { get; set; }

    public double? Dolar { get; set; }

    public double? PriseIqd { get; set; }

    public double? TotalIqd { get; set; }

    public double? SumTotalIqd { get; set; }

    public double? HamaliyaIqd { get; set; }

    public double? PayIqd { get; set; }

    public double? DiscontIqd { get; set; }

    public double? RemainIqd { get; set; }

    public string? Factoryy { get; set; }

    public decimal? Safiremain { get; set; }

    public decimal? LastRemain { get; set; }
}
